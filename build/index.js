...
function isListResourcesArgs(args) {
    if (typeof args !== 'object' || args === null)
        return false;
    const typedArgs = args;
    return (typeof typedArgs.category === 'string' &&
        typeof typedArgs.resource_type === 'string' &&
        Object.keys(RESOURCE_CATEGORIES).includes(typedArgs.category));
}
                ...
                switch (request.params.name) {
                    ...
                    case 'list_resources': {
                        ...
                        if (!isListResourcesArgs(rawArgs)) {
                            throw new McpError(ErrorCode.InvalidParams, 'Invalid arguments for list_resources');
                        }
                        ...
                        if (!categoryResources.includes(resource_type)) {
                            throw new McpError(ErrorCode.InvalidParams, `Invalid resource type for category ${category}: ${resource_type}`);
                        }
                    ...
                    case 'view_config_node_values': {
                        const { xpath } = request.params.arguments;
                    ...
                    case 'multi_move_clone_configuration': {
                        const { config_paths, new_location, action } = request.params.arguments;
                        const endpoint = action === 'move' ? 'MultiMove' : 'MultiClone';
                        const response = await this.axiosInstance.post(`/Configuration/${endpoint}`, { config_paths, new_location });
                        return {
                            content: [
                                {
                                    type: 'text',
